...
> **二进制怎么来的**：`vibeknow-cli` 把各平台的 Go 二进制打成独立的 npm 包（`@vectorfunc/vibeknow-cli-darwin-arm64` 等）放在 `optionalDependencies` 里，npm 按 `os`/`cpu` 只装匹配当前机器的那一个。所以二进制走的是**你已经在用的那个 registry**——公共镜像也好、公司内网代理也好——不需要额外一个主机可达。
>
